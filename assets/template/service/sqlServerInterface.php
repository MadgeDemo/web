<?php
	...
	// $webServiceUrl = 'http://www.essdp.com:8080/Service.svc?wsdl';//Live at Kippra
	 $webServiceUrl = 'http://www.essservice.com:8090/Service.svc?wsdl';
	...
	if(isset($_POST['action'])){
		...
		if($action == "REGISTER_USER"){
		 ...
		 //    $PersonID = $_POST['PersonID'];
		...
		}else if($action == "GETPERSONALENTITLEMENTS"){
			$client = new SoapClient($webServiceUrl);

			$PersonID = $_POST['PersonID'];
			$activeLeaveYear = $_POST['activeYear'];
			
			try {
				$res = $client->getPersonalEntitlements(array(
											"PersonID"=>$PersonID
											));
			} catch (SoapFault $e) {
			...
			// echo "<pre>";print_r($res);die();
			$entitlement = json_decode($res->getPersonalEntitlementsResult);
			
			try {
				$res = $client->getRequestedLeave(array(
											"PersonID"=>$PersonID,
											"LeaveYear"=>$activeLeaveYear
											));
			} catch (SoapFault $e) {
			...
			// echo "<pre>";print_r($res);die();
			$appliedLeave = json_decode($res->getRequestedLeaveResult);
			// echo "<pre>";print_r($appliedLeave);die();
			foreach ($entitlement as $value) {
				$loop1[] = $value->entitlementCode;
			}

			foreach ($appliedLeave as $key => $value) {
				$loop2[] = $value->CauseOfAbsence;
			}

			$result = array_values(array_diff($loop1, $loop2));
			
			foreach ($appliedLeave as $leaves) {
				foreach ($entitlement as $entitled) {
					if ($leaves->CauseOfAbsence == $entitled->entitlementCode) {
						$data[] = array(
							'daysAvaliable' => (int) $entitled->totalEntitlementForPayment - (int) $leaves->daysUsed,
							'daysUsed' => $leaves->daysUsed,
							'entitlementCode' => $entitled->entitlementCode,
							'totalEntitlementForPayment' => $entitled->totalEntitlementForPayment
						);
					}
				}
			}
			foreach ($result as $key => $value) {
				foreach ($entitlement as $entitled) {
					if ($value == $entitled->entitlementCode) {
						$data[] = array(
							'daysAvaliable' => (int) $entitled->totalEntitlementForPayment,
							'daysUsed' => (int) 0,
							'entitlementCode' => $entitled->entitlementCode,
							'totalEntitlementForPayment' => $entitled->totalEntitlementForPayment
						);
